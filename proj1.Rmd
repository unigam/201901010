...
1. ANOVA test of school level of education against total number of gang related crimes, total number of incidents reported with school and police (VIOINC10 and VIOPOL10)
```{r}
...
summary.aov(manova(cbind(dat$GANGHATE,dat$VIOINC10,dat$VIOPOL10)~fr_lvel)) 
```
2. ANOVA test of school enrollment size against total number of gang related crimes, total number of incidents reported with school and police (VIOINC10 and VIOPOL10).
```{r}
...
summary.aov(manova(cbind(dat$GANGHATE,dat$VIOINC10,dat$VIOPOL10)~fr_size)) 
```
3. ANOVA test of school location against total number of gang related crimes, total number of incidents reported with school and police (VIOINC10 and VIOPOL10)
```{r}
...
summary.aov(manova(cbind(dat$GANGHATE,dat$VIOINC10,dat$VIOPOL10)~fr_urban)) 
```
...
```
Let's change the factor levels to compare the other factors against one which has the largest number of observations as seen earlier from the bar plots in Section 2.1.2.1 and see how our results stack up!
```{r, results = "asis"}
perwht <- relevel(perwht, ref = 4)
fr_lvel <- relevel(fr_lvel, ref = 3)
fr_size <- relevel(fr_size, ref = 3)
fr_urban <- relevel(fr_urban, ref = 2)
m1<- lm(formula = GANGHATE~perwht+fr_lvel+fr_size+fr_urban, data = dat)
m2<- lm(formula = VIOINC10~perwht+fr_lvel+fr_size+fr_urban, data = dat)
m3<- lm(formula = VIOPOL10~perwht+fr_lvel+fr_size+fr_urban, data = dat)
stargazer(list(m1, m2, m3), type="html", single.row = TRUE,
covariate.labels = c("greater than 95% white enrollment","greater than 80% till 95% white enrollment","greater than 50% till 80% white enrollment","Primary Schools","Middle Schools","Combined Grade Offering","less than 300 enrollment size","300-499 enrollment size","1000 or more enrollment size","Schools in cities","Schools in Towns", "Schools in Rural Regions"),
    dep.var.labels = c("Total number of hate and gang related crimes reported", "Total number of violent crimes reported by the school","Total number of violent crimes reported by the police"),
          column.labels = c("Model 1", "Model 2","Model 3"))
```
